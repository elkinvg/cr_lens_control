...
folder are mapped to the application's root namespace, 'LensControl'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "LensControl"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'LensControl' in the same way
as `"LensControl/sass/src"`.
